...
def _make_app(tmp_path, monkeypatch, theme="textual-dark"):
    monkeypatch.setattr(
        ...
        lambda: AppSettings(db_path=tmp_path / "composition.db", theme=theme),
    )
...
async def test_choosing_a_theme_does_not_save_unsubmitted_db_path(
    tmp_path, monkeypatch, settings_path
):
    app = _make_app(tmp_path, monkeypatch)
    ...
    async with app.run_test() as pilot:
        radio_set = await _open_settings(app, pilot)
        app.screen.query_one("#db-path-input", Input).value = str(tmp_path / "other.db")

        radio_set.focus()
        await pilot.press("down", "down", "enter")
        await pilot.pause()

        assert load_settings(settings_path).db_path == tmp_path / "composition.db"


async def test_saving_db_path_keeps_chosen_theme(tmp_path, monkeypatch, settings_path):
    app = _make_app(tmp_path, monkeypatch, theme="composition-forest")

    async with app.run_test() as pilot:
        await _open_settings(app, pilot)
        db_input = app.screen.query_one("#db-path-input", Input)
        db_input.value = str(tmp_path / "other.db")
        db_input.focus()
        ...
        assert saved.db_path == tmp_path / "other.db"
